/* Main entry point */
#include "Yu_Gi_Oh__Dark_Duel_Stories__U___C____.h"
#include "gbrt.h"
...
int Yu_Gi_Oh__Dark_Duel_Stories__U___C_____main(int argc, char* argv[]) {
    ...
    const uint8_t* rom_data_override = NULL;
    size_t rom_size_override = 0;
...

    {
        ...
        rom_data_override = (const uint8_t*)malloc((size_t)rom_len);
        if (!rom_data_override) {
            fprintf(stderr, "Failed to allocate memory for ROM\n");
            fclose(rom_file);
            return 1;
        }
        if (fread((void*)rom_data_override, 1, (size_t)rom_len, rom_file) != (size_t)rom_len) {
            fprintf(stderr, "Failed to read ROM file\n");
            free((void*)rom_data_override);
            fclose(rom_file);
            return 1;
        }
        fclose(rom_file);
        rom_size_override = (size_t)rom_len;
        printf("[GBRT] Loaded ROM: %s (%zu bytes)\n", rom_path, rom_size_override);
    }
    ...
    GBConfig runtime_config = *Yu_Gi_Oh__Dark_Duel_Stories__U___C_____default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "Yu_Gi_Oh__Dark_Duel_Stories__U___C____");
        gb_context_set_save_id(interpreted_ctx, "Yu_Gi_Oh__Dark_Duel_Stories__U___C____");
        if (serial_stdout) {
            generated_ctx->callbacks.on_serial_byte = gbrt_serial_stdout;
        }
        if (rom_data_override) {
            Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init_with_rom(generated_ctx, rom_data_override, rom_size_override);
            Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init_with_rom(interpreted_ctx, rom_data_override, rom_size_override);
        } else {
            Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init(generated_ctx);
            Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init(interpreted_ctx);
        }
        if (data_mod_file) {
    ...
    gb_context_set_save_id(ctx, "Yu_Gi_Oh__Dark_Duel_Stories__U___C____");
    ...
    if (rom_data_override) {
        Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init_with_rom(ctx, rom_data_override, rom_size_override);
    } else {
        Yu_Gi_Oh__Dark_Duel_Stories__U___C_____init(ctx);
    }
    if (data_mod_file) {
    ...
    const GBPortMetadata port_metadata = {
        ...
        .game_id = "Yu_Gi_Oh__Dark_Duel_Stories__U___C____",
        .game_title = "Yu_Gi_Oh__Dark_Duel_Stories__U___C____",
        .rom_sha256 = "4336ce47861a57c2127aebb3d81cf02e4eb418bf01f3a342afca9f8dd3a1c618",
        .rom_size = 4194304u,
    };
    ...
    Yu_Gi_Oh__Dark_Duel_Stories__U___C_____run(ctx);
...
int main(int argc, char* argv[]) {
    return Yu_Gi_Oh__Dark_Duel_Stories__U___C_____main(argc, argv);
}
